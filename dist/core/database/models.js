...
export const UserModel = {
    ...
    async findById(id) {
        ...
        if (user)
            await cache.set(cacheKey, safeSerialize(user), 300);
    ...
    async findPaginated(limit = 50, offset = 0) {
...
export const UserBalanceModel = {
    // Get balance by user
    async findByUserId(userId) {
        const balances = await sql `SELECT * FROM user_balances WHERE user_id = ${userId}`;
    ...
    async getTotalPlatformBalance() {
        ...
        if (cached)
            return cached;
        const result = await sql `SELECT COALESCE(SUM(balance), 0) as total FROM user_balances`;
